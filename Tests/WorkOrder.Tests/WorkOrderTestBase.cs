...
namespace WorkOrder.Tests
{
    public abstract class WorkOrderTestBase : AbpIntegratedTestBase<WorkOrderTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected WorkOrderTestBase()
        {
        ...
        protected void UsingDbContext(Action<WorkOrderDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<WorkOrderDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<WorkOrderDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<WorkOrderDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<WorkOrderDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WorkOrderDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<WorkOrderDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WorkOrderDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<WorkOrderDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<WorkOrderDbContext, Task<T>> func)
        {
            T result;
